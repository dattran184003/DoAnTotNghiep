...
const BrandFeedback = () => {
    ...
    useEffect(() => {
        if (senderId) {
            axios.get(`https://localhost:7175/api/BrandFeedbacks/GetUserFeedback/${senderId}`)
                .then(response => {
    ...
    useEffect(() => {
        const fetchMessages = async () => {
            if (selectedUser && senderId) {
                try {
                    const response = await axios.get(`https://localhost:7175/api/BrandFeedbacks/GetBrandFeedbackMessages/${senderId}/${selectedUser.id}`);
    ...
    const sendFeedback = () => {
        ...
        axios.post('https://localhost:7175/api/BrandFeedbacks/SendBrandFeedback', feedback)
            .then(response => {
    ...
    <div className="account">
      ...
      <div className={`accountContainer ${isSidebarOpen ? "open" : ""}`}>
        <Navbar toggleSidebar={toggleSidebar} />
              <Container>
                  <h3>Danh sách phản hồi cửa hàng</h3>
                <Row>
                    <Col md={4}>
                        <Card>
                    ...
                    <Col md={8}>
                        <Card>
                            <Card.Header>Phản hồi thương hiệu từ {selectedUser ? selectedUser.userName : '...'}</Card.Header>
