...
const CarImagesList = () => {
    ...
    const [totalPages, setTotalPages] = useState(1); // Adjusted initial value
    ...
    const fetchCarImages = async () => {
        try {
            const response = await axios.get('https://localhost:7175/api/CarImages/GetCarImagesWithCarNames', {
                params: {
    ...
    const deleteCarImage = async (id) => {
        try {
            await axios.delete(`https://localhost:7175/api/CarImages/${id}`);
                    ...
                    <Table striped bordered hover responsive className="text-center">
                        <thead>
                            <tr>
                                <th>ID</th>
                                <th>Tên xe</th>
                                {Array.from({ length: 3 }).map((_, index) => ( // Displaying only first 3 images in the table header
                        ...
                        <tbody>
                            {carImages.map(carImage => (
                                <tr key={carImage.id}>
                                    ...
                                    {Array.from({ length: 3 }).map((_, index) => ( // Displaying only first 3 images in each table row
                                        <td key={index}>
                                            {carImage[`imageUrl${index + 1}`] && (
                                                <img
                                                    src={`https://localhost:7175${carImage[`imageUrl${index + 1}`]}`}
                                                    alt={`Ảnh ${index + 1}`}
